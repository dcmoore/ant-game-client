(ns cohort-ant-client.main-test
  (:require [clj-http.client :as client]
            [clojure.test :refer :all]
            [cohort-ant-client.main :refer :all]))
...
(deftest test-app
  ...
  (testing "move-ant will find food if it is nearby"
    (let [move-around-invocations (atom [])
          ant {:id "1234" :surroundings {"ne" [{:type :food}]}}]
      (with-redefs [move-around (mock-fn move-around-invocations)]
        (move-ant ant)
        (is (= [{:parameters [(:id ant) "ne"]}]
               @move-around-invocations)))))
  )
